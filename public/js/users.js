...
(function () {
  ...
  function renderUsers(users) {
    ...
    usersContainer.innerHTML = users
      .map((user, userIndex) => {
        ...
        return `
      <div class="bg-slate-900/50 border border-white/5 rounded-xl sm:rounded-2xl overflow-hidden profile-card" id="${userCardId}">
        <!-- User Header -->
        <div class="p-3 sm:p-4 lg:p-5 border-b border-white/5">
          <div class="flex items-start justify-between gap-2 sm:gap-3">
            <div class="flex items-start gap-2 sm:gap-3 lg:gap-4 flex-1 min-w-0">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-1.5 sm:gap-2 mb-1 flex-wrap">
                  ...
                  ${
                    user.is_verified
                      ? `<span class="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] font-medium text-emerald-400 bg-emerald-500/10 rounded-md sm:rounded-lg shrink-0">Verified</span>`
                      : `<span class="px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] font-medium text-amber-400 bg-amber-500/10 rounded-md sm:rounded-lg shrink-0">Unverified</span>`
                  }
                  ${
                    user.google_id
                      ? `<span class="google-badge px-1.5 sm:px-2 py-0.5 text-[9px] sm:text-[10px] font-medium rounded-md sm:rounded-lg shrink-0">Google</span>`
                      : ""
                  }
                </div>
                <div class="user-info-row flex flex-wrap items-center gap-1.5 sm:gap-2 lg:gap-3">
                  <span class="whitespace-nowrap">Status: <span class="text-white font-medium">${
                    user.status || "N/A"
                  }</span></span>
                  <span class="whitespace-nowrap hidden sm:inline">Role: <span class="text-white font-medium">${
                    user.role || "N/A"
                  }</span></span>
                  <span class="whitespace-nowrap hidden lg:inline">Language: <span class="text-white font-medium">${
                    user.preferred_language || "N/A"
                  }</span></span>
                  <span class="joined-date whitespace-nowrap">Joined: <span class="text-white font-medium">${formatDate(
                    user.created_at
                  )}</span></span>
                </div>
              </div>
            ...
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
              <div class="text-right hidden sm:block">
                ...
                <p class="text-[10px] sm:text-xs text-slate-400">Profile${
                  (user.totalProfiles || 0) !== 1 ? "s" : ""
                }</p>
              </div>
              <button
                onclick="toggleUserProfiles('${userCardId}')"
                class="p-1.5 sm:p-2 text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors"
                title="Toggle profiles"
              >
                <svg id="${userCardId}_icon" class="w-4 h-4 sm:w-5 sm:h-5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="divide-y divide-white/5">
        ${profiles
          .map((profile, profileIndex) => {
            ...
            return `
          <div class="p-4 sm:p-5 hover:bg-slate-800/30 transition-all duration-200 border-b border-slate-700/30 last:border-b-0">
            <div class="flex items-start gap-3 sm:gap-4">
              <!-- Profile Avatar -->
              <div class="relative shrink-0">
                <div class="profile-avatar rounded-xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 flex items-center justify-center text-white font-bold text-xs sm:text-sm shadow-lg shadow-purple-500/20 ring-2 ring-purple-500/30 overflow-hidden">
                  ${
                    avatarUrl
                      ? `<img src="${avatarUrl}" alt="${fullName}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                       <div class="w-full h-full items-center justify-center hidden bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500">${fullName
                         .charAt(0)
                         .toUpperCase()}</div>`
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-start justify-between gap-3 mb-2.5">
                  <div class="flex-1 min-w-0">
                    <h4 class="text-white font-medium text-sm sm:text-base mb-0.5 leading-snug truncate w-[30px] h-[30px]">${fullName}</h4>
                  ...
                  ${
                    ...
                    <div class="text-right shrink-0">
                      <span class="inline-flex items-center px-3 py-1.5 text-xs font-bold text-white bg-gradient-to-r from-emerald-500 to-emerald-600 border border-emerald-400/50 rounded-lg shadow-md shadow-emerald-500/20">${
                        membership.plan_name || "Member"
                      }</span>
                      <p class="text-xs text-slate-500 mt-2">Expires ${formatDate(
                        membership.expiry_date
                      )}</p>
                    </div>
                  `
                      : `
                    <span class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-slate-200 bg-slate-700/60 border border-slate-600/60 rounded-lg shrink-0">No Membership</span>
                  `
                  }
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-3">
                  <div>
                    <p class="text-[10px] sm:text-xs text-slate-400 mb-1 font-medium">Gender</p>
                    <p class="text-xs sm:text-sm text-white font-medium capitalize">${
                      profile.gender || "N/A"
                    }</p>
                  </div>
                  <div>
                    <p class="text-[10px] sm:text-xs text-slate-400 mb-1 font-medium">Date of Birth</p>
                    <p class="text-xs sm:text-sm text-white font-medium">${formatDOB(
                      profile.dob
                    )}</p>
                  </div>
                  <div>
                    <p class="text-[10px] sm:text-xs text-slate-400 mb-1 font-medium">Location</p>
                    <p class="text-xs sm:text-sm text-white font-medium truncate">${
              ...
              profile.address?.country ? `, ${profile.address.country}` : ""
                  ...
                  <div>
                    <p class="text-[10px] sm:text-xs text-slate-400 mb-1 font-medium">QR Items</p>
                ...
                ${
                  profile.medical_information
                    ? `
                  <div class="grid grid-cols-2 sm:flex sm:flex-wrap gap-1.5 sm:gap-2 mb-3 px-0 sm:px-1 py-1">
                    ${
                      profile.medical_information.blood_type
                        ? `<span class="inline-flex items-center justify-center px-2 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold text-white bg-gradient-to-r from-red-600 to-rose-600 border-2 border-red-400/50 rounded-md sm:rounded-lg shadow-md shadow-red-500/30 whitespace-nowrap">Blood: ${profile.medical_information.blood_type}</span>`
                        : ""
                    }
                    ${
                      (profile.medical_information.allergies || []).length > 0
                        ? `<span class="inline-flex items-center justify-center px-2 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs font-semibold text-white bg-orange-500/30 border border-orange-400/50 rounded-md sm:rounded-lg whitespace-nowrap">${profile.medical_information.allergies.length} Allergies</span>`
                        : ""
                    }
                    ${
                      (profile.medical_information.chronic_diseases || [])
                        .length > 0
                        ? `<span class="inline-flex items-center justify-center px-2 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs font-semibold text-white bg-amber-500/30 border border-amber-400/50 rounded-md sm:rounded-lg whitespace-nowrap">${profile.medical_information.chronic_diseases.length} Conditions</span>`
                        : ""
                    }
                    ${
                      (profile.medical_information.medications || []).length > 0
                        ? `<span class="inline-flex items-center justify-center px-2 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs font-semibold text-white bg-blue-500/30 border border-blue-400/50 rounded-md sm:rounded-lg whitespace-nowrap">${profile.medical_information.medications.length} Medications</span>`
                        : ""
                    }
                  </div>
                `
                ...
                <!-- Stats Row -->
                <div class="flex flex-wrap items-center gap-3 sm:gap-4 text-xs text-slate-400 pt-3 border-t border-slate-700/30 mb-3">
                  ${
                    (profile.medical_history || []).length > 0
                      ? `<span class="whitespace-nowrap">History: <span class="text-white font-medium">${profile.medical_history.length}</span></span>`
                      : ""
                  }
                  ${
                    (profile.medical_documents || []).length > 0
                      ? `<span class="whitespace-nowrap">Docs: <span class="text-white font-medium">${profile.medical_documents.length}</span></span>`
                      : ""
                  }
                  ${
                    (profile.emergency_details?.contacts || []).length > 0
                      ? `<span class="whitespace-nowrap hidden sm:inline">Emergency: <span class="text-white font-medium">${profile.emergency_details.contacts.length}</span></span>`
                      : ""
                  }
                  ${
                    qrItems.length > 0
                      ? `<span class="whitespace-nowrap">QR: <span class="text-white font-medium">${qrItems.length}</span></span>`
                      : ""
                  }
                </div>
                ...
                <button
                  ...
                  class="w-full px-4 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 rounded-lg transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transform hover:scale-[1.02]"
                >
  ...
  window.viewProfileDetails = function (profileId) {
    ...
    if (membership && membership.isActive) {
      ...
      badges.push(`
        <span class="px-2 sm:px-3 lg:px-4 py-1 sm:py-1.5 lg:py-2 text-[10px] sm:text-xs font-semibold rounded-lg sm:rounded-xl bg-gradient-to-r from-emerald-500/25 to-emerald-600/25 border border-emerald-500/50 text-emerald-200 flex items-center gap-1 sm:gap-2 shadow-lg shadow-emerald-500/15 ${
    ...
    if (medicalInfo.blood_type) {
      badges.push(`
        <span class="px-2 sm:px-3 lg:px-4 py-1 sm:py-1.5 lg:py-2 text-xs sm:text-sm font-bold rounded-lg sm:rounded-xl bg-gradient-to-r from-red-600/40 to-rose-600/40 border-2 border-red-400/70 text-white shadow-xl shadow-red-500/30 flex items-center justify-center min-w-[40px] sm:min-w-[50px] backdrop-blur-sm">
    ...
    modalTabs.innerHTML = tabs
      ...
      <button
        ...
        class="tab-button relative px-3 sm:px-4 lg:px-5 py-2 sm:py-2.5 lg:py-3.5 text-xs sm:text-sm font-semibold text-slate-400 hover:text-white transition-all flex items-center gap-1.5 sm:gap-2 lg:gap-2.5 rounded-t-lg sm:rounded-t-xl whitespace-nowrap shrink-0 ${
          index === 0 ? "active" : ""
        }"
        data-tab="${tab.id}"
      >
        <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 lg:w-4.5 lg:h-4.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
